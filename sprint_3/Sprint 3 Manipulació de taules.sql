...
La teva tasca és dissenyar i crear una taula anomenada "credit_card" que emmagatzemi detalls crucials sobre les targetes de crèdit.
 ...
 targeta i establir una relació adequada amb les altres dues taules ("transaction" i "company"). 
...
-- remove all data from credit_card
TRUNCATE TABLE credit_card; 

-- extend longitude of iban column and then rerun insert
ALTER TABLE credit_card MODIFY COLUMN iban VARCHAR(50);

-- check that data is added to table
SELECT *
FROM credit_card;

-- check that there is no the foreign key 
SELECT TABLE_NAME, COLUMN_NAME, CONSTRAINT_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME 
FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
WHERE TABLE_NAME = 'transaction' AND REFERENCED_TABLE_NAME = 'credit_card';
...
*/
-- find current values  for the user with id = "CcU-2938"
SELECT *
FROM credit_card
WHERE id = "CcU-2938";

-- update iban value for the user with id = "CcU-2938"
UPDATE credit_card
SET iban = "R323456312213576817699999"
WHERE id = "CcU-2938";

-- check if changes are implemented by selcting the user with id = "CcU-2938"  and the updated iban value
SELECT *
FROM credit_card
WHERE id = "CcU-2938" AND iban = "R323456312213576817699999";
...
-- check if company with id "CcU-9999' exists in table company
SELECT *
FROM company 
WHERE id = 'b-9999'; -- otput it does not exist

-- insert company with id 'b-9999' to company table 
INSERT INTO company (id)
VALUES ('b-9999');

-- check if credit card witrh id "CcU-9999' exists in table credit_card
SELECT *
FROM credit_card
WHERE id = 'CcU-9999'; 

-- insert credit acrd with id 'CcU-9999' to credit_card table 
...
-- delete the column "pan" from the credit_card table
ALTER TABLE credit_card DROP COLUMN pan;

-- verify if the column has been successfully deleted by checking the table schema
SELECT column_name
...
-- check that transaction with id "02C6201E-D90A-1859-B4EE-88D2986D3B02" exists in table transaction
SELECT *
FROM transaction
WHERE id = "02C6201E-D90A-1859-B4EE-88D2986D3B02";

-- verifico si transaction 
SELECT TABLE_NAME, COLUMN_NAME, CONSTRAINT_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME
FROM information_schema.KEY_COLUMN_USAGE 
WHERE REFERENCED_TABLE_NAME = 'transaction' 
AND REFERENCED_COLUMN_NAME = 'id';

-- delete transaction with id = "02C6201E-D90A-1859-B4EE-88D2986D3B02"
DELETE FROM transaction WHERE id = "02C6201E-D90A-1859-B4EE-88D2986D3B02";

-- check that transaction with id = "02C6201E-D90A-1859-B4EE-88D2986D3B02" was deleted
SELECT *
FROM transaction
WHERE id = "02C6201E-D90A-1859-B4EE-88D2986D3B02";
...
--  checked if there are null and 0 values in the column amount before creation of view
SELECT COUNT(*) AS total_records, 
       COUNT(t.amount) AS non_null_records,
       SUM(CASE WHEN t.amount = 0 THEN 1 ELSE 0 END) AS zero_values
FROM company c 
INNER JOIN transaction t WHERE c.id = t.company_id;

-- create view VistaMarketingvistamarketing
CREATE VIEW VistaMarketing AS
SELECT c.company_name, c.phone, c.country, AVG(t.amount) AS average_purchase
FROM company c 
INNER JOIN transaction t WHERE c.id = t.company_id 
-- exclude transactions that declined
AND declined = 0
GROUP BY c.id
...
-- remove column website from table company
ALTER TABLE company DROP COLUMN website;

-- check that column website is removed
SELECT *
FROM company;

-- ensure all existing PIN values fit within VARCHAR(4) before altering the column 
SELECT LENGTH(pin) FROM credit_card GROUP BY LENGTH(pin);

-- change the data type of pin from INT to VARCHAR(4)
ALTER TABLE credit_card MODIFY COLUMN pin VARCHAR(4);

-- check the structure of the credit_card table to see the data type of the pin column. 
SHOW COLUMNS FROM credit_card;

-- change the data type of id from VARCHAR(15) to VARCHAR(20)
ALTER TABLE credit_card MODIFY COLUMN id VARCHAR(20);

-- change the data type of expiring_date from VARCHAR(8) to VARCHAR(20)
ALTER TABLE credit_card MODIFY COLUMN expiring_date VARCHAR(20);

-- check the structure of the credit_card table to see the data type of the pin column. 
SHOW COLUMNS FROM credit_card;

-- add the column Fecha_actual with the data type DATE
...
-- populate column fecha_actual with current date
UPDATE credit_card 
...
-- check that fecha_actual is added
SELECT *
FROM credit_card;
...
-- change table name from user to data_user
...
-- check FK of transaction table
SELECT TABLE_NAME, COLUMN_NAME, CONSTRAINT_NAME, 
       REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME
FROM information_schema.KEY_COLUMN_USAGE
WHERE TABLE_NAME = 'transaction' AND REFERENCED_TABLE_NAME IS NOT NULL;

-- add FK to transaction.user_id linking it to data_user.id
ALTER TABLE transaction 
ADD CONSTRAINT fk_transaction_user
FOREIGN KEY (user_id) REFERENCES data_user(id)
ON DELETE CASCADE ON UPDATE CASCADE;

-- find user_id values in transaction that do not exist in data_user
SELECT DISTINCT user_id 
FROM transaction 
WHERE user_id NOT IN (SELECT id FROM data_user);

-- remove tranactions with user_id that does not exist in data_user from transaction
DELETE FROM transaction
WHERE user_id = 9999;

-- find the name of the foreign key 
SELECT CONSTRAINT_NAME 
FROM information_schema.KEY_COLUMN_USAGE 
WHERE TABLE_NAME = 'data_user' AND COLUMN_NAME = 'id';

-- drop the incorrect foreign key constraint
ALTER TABLE data_user DROP FOREIGN KEY data_user_ibfk_1;

-- check that incorrect foreign key constraint data_user_ibfk_1 was dropped 
SELECT CONSTRAINT_NAME 
FROM information_schema.KEY_COLUMN_USAGE 
...
SELECT 
DISTINCT transaction.id AS transaction,
data_user.name AS name,
data_user.surname AS surname,
...
-- check that view InformeTecnico is craeted
SELECT *
FROM InformeTecnico; 
